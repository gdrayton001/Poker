from PokerCalculator import toString
from tkinter import *
import PIL.Image
import PIL.ImageTk
...
class Player:
    ...
    def __init__(self, order, label, root):
        self.order = order
        self.label = Label(root, text=label + '\n chips: ' + toString(self.chips - self.betting))
        self.name = label
        self.bettingIcon = Label(root, text=toString(self.betting)) 
    ...
    def bet(self, amount):
        ...
        self.betting += amount
        self.bettingIcon.config(text=toString(self.betting))
