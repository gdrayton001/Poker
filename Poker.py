...
def endRound():
    ...
    if roundEnded > 0:
        pot_label.config(text=str(game.pot))
    else:
        pot_label = Label(root, text=str(game.pot))
...
def minusRaiseBet():
    amount_betting_label.config(text=int(amount_betting_label.cget('text'))-game.blind)
    plus_button.config(state=NORMAL)
    if int(amount_betting_label.cget('text'))-game.blind <= max(game.blind, game.betAmount):
...
def nextHand():
    ...
    limp_button = Button(root, text="Bet " + str(min(game.betAmount-game.activePlayer.betting, game.activePlayer.chips-game.activePlayer.betting)), command=lambda: placeBet(game.activePlayer, min(game.betAmount-game.activePlayer.betting, game.activePlayer.chips-game.activePlayer.betting)), state=NORMAL)
    ...
    strength_label = Label(root, text=str(getHandStrength(game.activePlayer.hand)) + "/10")
...
def nextTurn(newOrder): 
    ...
    strength_label.config(text=str(getHandStrength(game.activePlayer.hand)) + "/10")
    strength_label.grid_forget()
    if game.betAmount > game.activePlayer.betting:
        limp_button.config(text="Bet " + str(min(game.betAmount-game.activePlayer.betting, game.activePlayer.chips-game.activePlayer.betting)), command=lambda: placeBet(game.activePlayer, min(game.betAmount-game.activePlayer.betting, game.activePlayer.chips-game.activePlayer.betting)))
    else:
...
def placeBet(player, amount):
    ...
    #offset = 1
    #while not game.getPlayerWithOrder((player.order+1) % len(game.players)).active:
        #offset += 1
    newOrder = game.nextTurn((player.order+1) % len(game.players))
    if newOrder == -1:
        endRound()
...
def plusRaiseBet():
    amount_betting_label.config(text=min(int(amount_betting_label.cget('text'))+game.blind, game.activePlayer.chips-game.activePlayer.betting))
    minus_button.config(state=NORMAL)
    if int(amount_betting_label.cget('text')) == game.activePlayer.chips-game.activePlayer.betting:
...
def raiseBet(player):
    ...
    amount_betting_label = Label(raise_bet_window, text=str(game.betAmount + game.blind))
    minus_button = Button(raise_bet_window, text="-", command=minusRaiseBet, state=DISABLED)
    submit_raised_bet_button = Button(raise_bet_window, text="Submit", command=lambda: submitRaisedBet(game.activePlayer, int(amount_betting_label.cget('text'))))
...
def start_game():
    ...
    for i in range(num_users.get()):
        players.append(Player(i, "P"+str(i+1), root))
    for i in range(num_cpus.get()):
        players.append(CPU(i+num_users.get(), "CP"+str(i+1), root))
    if len(players) < 2:
